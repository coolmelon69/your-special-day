...
export type Tier = AchievementTierProgress["tier"];
...
export const TRACK_SHAPE: Record<string, string> = {
...
export const FALLBACK_SHAPE = TRACK_SHAPE.PenLine;

export const shapeFor = (iconKey: string): string => TRACK_SHAPE[iconKey] ?? FALLBACK_SHAPE;
...
const SHEEN_DELAY: Record<Tier, number> = {
  bronze: 0,
  silver: 0.55,
  gold: 1.1,
};
...
export interface PinBadgeProps {
  ...
  /** Edge length in px. Omitted, the badge keeps its responsive grid size. */
  size?: number;
  /** Drops the cast shadow and seated glow — for use inside the 3D medal stack,
   *  where depth comes from the extruded rim and a ground shadow instead. */
  flat?: boolean;
}

/** The struck pin: metal rim, recessed enamel plate, bevel light along the top-left. */
const PinBadge = ({
  tier,
  iconKey,
  Icon,
  ...
  return (
    <div
      className={cn("relative", size === undefined && "h-20 w-20 sm:h-24 sm:w-24")}
      style={size === undefined ? undefined : { width: size, height: size }}
    >
      {/* the pin clasp, catching its own highlight */}
      <div
        className={cn(
          "absolute left-1/2 top-0 z-10 -translate-x-1/2 -translate-y-[45%] rounded-full border",
          size === undefined && "h-3 w-3",
          locked
            ? "border-border bg-muted"
            : "border-black/15 bg-[radial-gradient(circle_at_32%_28%,#fff,rgba(120,120,130,0.85))] shadow-[0_1px_2px_rgba(0,0,0,0.25)]"
        )}
        style={size === undefined ? undefined : { width: size * 0.13, height: size * 0.13 }}
      />
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        {locked && !showProgress ? (
          <Lock
            className={cn("text-muted-foreground/45", size === undefined && "h-4 w-4 sm:h-[1.15rem] sm:w-[1.15rem]")}
            style={size === undefined ? undefined : { width: size * 0.19, height: size * 0.19 }}
            aria-hidden
          />
        ) : locked ? (
          // The icon fills with the badge: ghosted above the waterline, struck white below it.
          <div
            className={cn("relative", size === undefined && "h-7 w-7 sm:h-8 sm:w-8")}
            style={size === undefined ? undefined : { width: size * 0.33, height: size * 0.33 }}
          >
          ...
          <Icon
            className={cn(
              "text-white drop-shadow-[0_1px_1.5px_rgba(0,0,0,0.45)]",
              size === undefined && "h-7 w-7 sm:h-8 sm:w-8"
            )}
            style={size === undefined ? undefined : { width: size * 0.33, height: size * 0.33 }}
            aria-hidden
          />
